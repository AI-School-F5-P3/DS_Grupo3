...
def screen_predict():
    ...
    if st.button("Predecir Riesgo de Ictus"):
        ...
        inputs_woe = transform_to_woe(inputs[category_columns], dict_woe)
        
        # Get prediction using the optimized model
        probabilities = st.session_state.model.predict(inputs_woe)[0][1]
        final_prediction = 1 if probabilities >= 0.5 else 0
        ...
        firebase_data = {
            ...
            "prediction_probability": float(probabilities)
        }

        # Save prediction and metrics
        save_prediction_to_firebase(firebase_data)
        metrics_data = {
            "avg_prediction": float(probabilities),
            "entropy": float(-np.sum(probabilities * np.log(probabilities)) if probabilities > 0 else 0),
            "ks_statistic": float(np.max(probabilities))
        }
        ...
        fig = create_gauge_chart(float(probabilities) * 100, "Probabilidad de Ictus")
